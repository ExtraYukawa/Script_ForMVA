//Code generated automatically by TMVA for Inference of Model file [DNN_Flav_MET_threeJet_model_v2.onnx] at [Thu Jul  6 00:14:39 2023] 

#ifndef TMVA_SOFIE_DNN_FLAV_MET_THREEJET_MODEL_V2
#define TMVA_SOFIE_DNN_FLAV_MET_THREEJET_MODEL_V2
...
namespace TMVA_SOFIE_DNN_Flav_MET_threeJet_model_v2{
namespace BLAS{
...
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(1056);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_Flav_MET_threeJet_model_v2.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 1056) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 1056 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
} //TMVA_SOFIE_DNN_Flav_MET_threeJet_model_v2

#endif  // TMVA_SOFIE_DNN_FLAV_MET_THREEJET_MODEL_V2
