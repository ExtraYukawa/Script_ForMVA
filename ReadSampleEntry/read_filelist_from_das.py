...
def read_filelist_from_das(nick, query, outputfile, phys03, xootd_prefix, era):
    ...
    for file in filedict.keys():
        ...
        count = count + 1 
    outfile.write("cd /tmp/{username}/{name} \n".format(
          path=file, username=getpass.getuser(), name=nick
        )
    )
    outfile.write("cp %s/src/Script_ForMVA/ReadSampleEntry/read_nevent.py .\n" %cmsswBase)
    outfile.write("wget https://raw.githubusercontent.com/ExtraYukawa/ttc_bar/lep_mvaID/scripts/haddnano.py\n")
    outfile.write("python haddnano.py output.root *.root \n")
    outfile.write("python read_nevent.py --nick=%s \n"%nick)
    outfile.write("cp nevents.json %s/src/Script_ForMVA/ReadSampleEntry/data/%s/%s.json\n"%(cmsswBase,era,nick))
    outfile.write("rm *\n")
    outfile.close()
...
if __name__ == "__main__":
    ...
    read_filelist_from_das(
        args.nick, args.query, args.outputfile, args.phys03, xootd_prefix, args.era
    )
